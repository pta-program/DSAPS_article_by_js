...
// 分别计算四种榜单的并列名次，查询时选择名次最小、编号最小的榜单。
const [n, m] = [Number(tokens[at++]), Number(tokens[at++])],
  countries = [];
for (let i = 0; i < n; i++)
  countries.push({
    gold: Number(tokens[at++]),
    medal: Number(tokens[at++]),
    population: Number(tokens[at++]),
    id: i,
  });
const metrics = [
    (c) => c.gold,
    (c) => c.medal,
    (c) => (c.population ? c.gold / c.population : c.gold * 1e9),
    (c) => (c.population ? c.medal / c.population : c.medal * 1e9),
  ],
  ranks = [];
for (const metric of metrics) {
  const order = countries.slice().sort((a, b) => metric(b) - metric(a));
  const rank = Array(n);
  order.forEach((c, i) => {
    rank[c.id] =
      i && metric(c) === metric(order[i - 1]) ? rank[order[i - 1].id] : i + 1;
  });
  ranks.push(rank);
}
const out = [];
for (let i = 0; i < m; i++) {
  const id = Number(tokens[at++]);
  let bestRank = Infinity,
    bestMethod = 0;
  for (let method = 0; method < 4; method++)
    if (ranks[method][id] < bestRank) {
      bestRank = ranks[method][id];
      bestMethod = method + 1;
    }
  out.push(`${bestRank}:${bestMethod}`);
}

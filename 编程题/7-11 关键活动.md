...
// AOE 网：正向拓扑求 ve，逆拓扑求 vl，满足 ve[u] + w === vl[v] 的边即关键活动。
...
for (let h = 0; h < queue.length; h++) {
  ...
  for (const e of graph[u]) {
    ve[e.v] = Math.max(ve[e.v], ve[u] + e.w);
    if (--indegree[e.v] === 0) queue.push(e.v);
  }
}
if (topo.length !== N) {
  console.log("0");
} else {
  const duration = Math.max(...ve),
    vl = Array(N + 1).fill(duration);
  for (let i = topo.length - 1; i >= 0; i--) {
    const u = topo[i];
    for (const e of graph[u]) vl[u] = Math.min(vl[u], vl[e.v] - e.w);
  }
  const critical = edges
    .filter((e) => ve[e.u] + e.w === vl[e.v])
    .sort((a, b) => a.u - b.u || b.i - a.i);
  console.log([duration, ...critical.map((e) => `${e.u}->${e.v}`)].join("\n"));
}
```

...
// 对每个序列模拟 BST 搜索路径；访问顺序完全相同才是同一棵 BST。
function pathOf(sequence) {
  const root = { value: sequence[0], left: null, right: null };
  const path = new Set([sequence[0]]);
...
while (at < tokens.length) {
  const n = Number(tokens[at++]),
    l = Number(tokens[at++]);
  if (!n) break;
  const base = Array.from({ length: n }, () => Number(tokens[at++]));
  const expected = pathOf(base);
  const out = [];
  for (let i = 0; i < l; i++) {
    const seq = Array.from({ length: n }, () => Number(tokens[at++]));
    out.push(pathOf(seq).every((v, j) => v === expected[j]) ? "Yes" : "No");
  }

...
// 服务队列保存顾客下标；start 单调前进，保证夹塞查找不会重复扫描。
const n = Number(tokens[at++]),
  groups = new Map();
const groupCount = Number(tokens[at++]);
for (let g = 0; g < groupCount; g++) {
  const size = Number(tokens[at++]);
  for (let j = 0; j < size; j++) groups.set(tokens[at++], g);
}
const customers = [];
for (let i = 0; i < n; i++)
  customers.push({
    name: tokens[at++],
    arrive: Number(tokens[at++]),
    process: Math.min(60, Number(tokens[at++])),
  });
const queue = [0],
  visited = Array(n).fill(false),
  order = [],
  groupOf = (name) => (groups.has(name) ? groups.get(name) : -1);
visited[0] = true;
let front = 0,
  idx = 0,
  start = 1,
  time = customers[0].arrive + customers[0].process,
  totalWait = 0;
while (front < queue.length) {
  const currentIndex = queue[front++],
    current = customers[currentIndex];
  order.push(current.name);
  const currentGroup = groupOf(current.name);
  let found = false;
  for (let i = start; i < n; i++) {
    ...
    if (customers[i].arrive > time) break;
    if (currentGroup >= 0 && groupOf(customers[i].name) === currentGroup) {
      visited[i] = true;
      totalWait += time - customers[i].arrive;
      time += customers[i].process;
      queue.push(i);
      start = i + 1;
      found = true;
      break;
  ...
  for (let i = idx + 1; i < n; i++)
    if (!visited[i]) {
      idx = i;
      visited[i] = true;
      time = Math.max(time, customers[i].arrive);
      totalWait += time - customers[i].arrive;
      time += customers[i].process;
      queue.push(i);
      start = idx + 1;

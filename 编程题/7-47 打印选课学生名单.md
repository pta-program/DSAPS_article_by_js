...
// 建立课程到学生的倒排表；输入学生按课程编号升序输出姓名。
const [n, k] = [Number(tokens[at++]), Number(tokens[at++])],
  courses = Array.from({ length: k + 1 }, () => []);
for (let i = 0; i < n; i++) {
  const name = tokens[at++],
    count = Number(tokens[at++]);
  for (let j = 0; j < count; j++) courses[Number(tokens[at++])].push(name);
}
const out = [];
for (let course = 1; course <= k; course++) {
  courses[course].sort();
  out.push(`${course} ${courses[course].length}`, ...courses[course]);
}

...
- 文件 3 的词汇集合：{Aaa, ccc, Eee, Ddd, Fff}（数字是分隔符，is/at 长度不足 3）
- 查询 1 2：公共词汇 {Bbb, Ccc}，总词汇 {Aaa, Bbb, Ccc, Ddd}，相似度 = 2/4 = 50.0%
- 查询 1 3：公共词汇 {Aaa, Ccc}，总词汇 {Aaa, Bbb, Ccc, Eee, Ddd, Fff}，相似度 = 2/6 ≈ 33.3%
...
for (let i = 0; i < n; i++) {
  ...
  while (lineAt < lines.length && lines[lineAt] !== "#") {
    for (const word of lines[lineAt++].toLowerCase().match(/[a-z]+/g) || [])
      if (word.length >= 3) words.add(word.slice(0, 10));
  }
  lineAt++;
  files.push(words);
}

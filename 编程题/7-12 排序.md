...
本题采用 JavaScript 的 `Array.prototype.sort` 升序排序，围绕题目给出的数据结构和约束完成核心计算，并处理边界情况。

## 代码流程说明

1. 读取题目规定的输入数据。
2. 使用带数值比较器的 `Array.prototype.sort` 完成主要处理。
3. 处理边界情况并整理结果。
4. 按指定格式输出结果。
...
// JavaScript 数值排序必须提供比较函数，否则会按字符串排序。
const n = Number(tokens[at++]);
const values = Array.from({ length: n }, () => BigInt(tokens[at++]));

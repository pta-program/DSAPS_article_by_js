...
```javascript
const ERROR = 1e8;
// 一个数组从两端向中间增长：Top1 指向左栈栈顶，Top2 指向右栈栈顶下一格。
```
...
## 裁判测试程序样例
```javascript
const S = CreateStack(5);
Push(S, 1, 1);
Push(S, 2, 2);
console.log(Pop(S, 1), Pop(S, 2));
```
...
```javascript
// 两个栈共享同一段空间，Top1 + 1 === Top2 表示空间已耗尽。
function CreateStack(MaxSize) {
  return { Data: Array(MaxSize), Top1: -1, Top2: MaxSize, MaxSize };
}
function Push(S, X, Tag) {
...
function Pop(S, Tag) {
  if (Tag === 1) return S.Top1 < 0 ? ERROR : S.Data[S.Top1--];
  return S.Top2 === S.MaxSize ? ERROR : S.Data[S.Top2++];
}
```
